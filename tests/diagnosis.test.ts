...
describe("業務改善助成金", () => {
  ...
  it("中小企業規模を超過（資本金・従業員とも超過）すると○にはならない", () => {
    const r = runDiagnosis({ ...base, industryCategory: "小売業", employeeCount: 80, capitalYen: 100000000 });
    expect(get("gyomukaizen", r).status).not.toBe("○");
  });

  it("事業場内最低賃金が地域別最低賃金以上だと○にならない", () => {
    const r = runDiagnosis({ ...base, inHouseMinWage: 1300, regionalMinWage: 1226 });
    expect(get("gyomukaizen", r).status).not.toBe("○");
  });
});
...
describe("働き方改革推進支援助成金（労働時間短縮・年休促進支援コース）", () => {
  ...
  it("取り組みたいこと（成果目標）は任意：未選択でも他要件を満たせば○", () => {
    const r = runDiagnosis({ ...base, hatarakiGoal1: false, hatarakiGoal2: false, hatarakiGoal3: false });
    expect(get("hatarakikata", r).status).toBe("○");
  });

  it("改善事業（設備）が無ければ△", () => {
    const r = runDiagnosis({ ...base, willInvestEquipment: false, equipmentName: "" });
    expect(get("hatarakikata", r).status).toBe("△");
  });
...
describe("設定ファイル駆動（法改正対応）", () => {
  it("中小企業の規模上限を変更すると業務改善助成金の判定が変わる", () => {
    const input: DiagnosisInput = { ...base, industryCategory: "小売業", employeeCount: 80, capitalYen: 100000000 };
    const c = getCriteria();
